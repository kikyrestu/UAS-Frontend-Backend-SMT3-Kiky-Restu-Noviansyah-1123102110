        ...
        if parsed_path.path == '/api/routes':
            ...
            if schedule_id:
                ...
                c.execute("""UPDATE schedules 
                             SET route_id=?, bus_id=?, departure_date=?, departure_time=?, price=?, available_seats=?
                             WHERE id=?""", 
                          (route_id, bus_id, departure_date, departure_time, price, available_seats, schedule_id))
                conn.commit()
                conn.close()
                self.send_response(200)
                self.end_headers()
